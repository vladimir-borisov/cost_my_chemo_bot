...
WELCOME = """
Здравствуйте! Это чат-бот онкологической «Клиники доктора Ласкова» (hemonc.ru). 
Я предназначен для оценки стоимости химиотерапии в нашей клинике.
❗*Важно*: с моей помощью пока можно рассчитать не все курсы химиотерапии, которую мы проводим в клинике.
Если вы не нашли своего курса, пришлите запрос на info@hemonc.ru и мы пришлём вам расчёт на почту.

Нажмите кнопку, чтобы начать.
"""

START = dedent("""
Спасибо. Для расчёта понадобятся рост и вес человека, который будет лечиться, и схема лечения/назначенный препарат. 

❗*Важно*: в результате расчета вы получите стоимость одного курса. 

Что такое «курс» в химиотерапии? Это одно введение препаратов. Обычно курсы химиотерапии проводят раз в 1-3 недели в зависимости от схемы. В выписке чаще всего бывает указано, как часто должны проводиться курсы и сколько их нужно.

Стоимость курса, которую мы будем рассчитывать, включает все необходимые по схеме препараты для одного введения, премедикацию (препараты от тошноты), проведение сеанса химиотерапии в дневном стационаре, наблюдение врача-онколога и медицинской сестры. То есть, это будет полная стоимость одного сеанса химиотерапии в «Клинике доктора Ласкова».
...
Нужно округлить до целого значения, единицы измерения не нужны.
Например: 170
"""
HEIGHT_WRONG = """
Это не целое число 🧐
Пожалуйста, введите рост в сантиметрах (только число). 
Например: 170
"""
...
WEIGHT_WRONG = """
Это не целое число 🧐
Пожалуйста, введите вес в килограммах (только число). 
Например: 75
"""
...
CUSTOM_COURSE_INPUT = """
Напишите название курса, который вы искали, или названия препаратов. 
Мы рассчитаем их стоимость и пришлём вам эту информацию.
"""

DATA_CONFIRMATION = dedent(
    """
    Спасибо! Вы указали все нужные данные. 
    Проверьте, пожалуйста, всё ли так? 
    Рост: {height} см
    Вес: {weight} кг
...
DATA_CORRECT = dedent(
    """
    Отлично! Стоимость курса (одного сеанса химиотерапии) составит: {course_price}
    Она может изменяться в зависимости от закупочной цены препарата.
